...
const props = defineProps(['brandVoices', 'categories', 'total', 'active', 'inActive', 'buttons'])
...
const store = () => {
  form.post(route('admin.brand-voices.store'), {
    onSuccess: () => {
      form.reset()
...
const openEditItemDrawer = (item) => {
  ...
  editForm.tones = item.tones
  editForm.status = item.status
  drawer.of('#editItemDrawer').show()
}

const update = () => {
  editForm.put(route('admin.brand-voices.update', editForm.id), {
    onSuccess: () => {
      editForm.value = {}
    ...
    <div class="space-y-6">
      ...
      <div class="table-responsive whitespace-nowrap rounded-primary">
        <table class="table">
          ...
          <tbody v-if="brandVoices.total">
            <tr v-for="brandVoice in brandVoices.data" :key="brandVoice.id">
              <td>
                {{ brandVoice.category?.title }}
              </td>

              <td class="text-left" v-for="tone in brandVoice.tones" :key="tone">
                {{ textExcerpt(tone?.text ?? '', 30) }}
              </td>

              <td class="text-left">
                <span class="badge" :class="brandVoice.status == 'active' ? 'badge-success' : 'badge-danger'">
                  {{ brandVoice.status == 'active' ? trans('Active') : trans('Draft') }}
                </span>
              </td>
              <td>
                {{ moment(brandVoice.created_at).format('D-MMM-Y') }}
              </td>
              <td class="">
                  ...
                  <div class="dropdown-content w-40">
                    <ul class="dropdown-list">
                      <li class="dropdown-list-item">
                        <button @click="openEditItemDrawer(brandVoice)" class="dropdown-link">
                          <Icon icon="fe:edit" />
                      ...
                      <li class="dropdown-list-item">
                        <button as="button" class="dropdown-link"
                          @click="deleteRow(route('admin.brand-voices.destroy', brandVoice.id))">
                          <Icon icon="fe:trash" />
        ...
        <Paginate :links="brandVoices.links" />
      </div>
    </div>
  </main>
  ...
  <div id="addNewItemDrawer" class="drawer drawer-right w-[600px]">
    <form @submit.prevent="store">
      <div class="drawer-header">
        <h5>{{ trans('Add New Brand Voice') }}</h5>
        <button type="button"
      ...
      <div class="drawer-body">
        <div class="mb-2">
          <label class="label mb-2">{{ trans('Category') }}</label>
        ...
        <div class="mb-2">
          ...
          <input v-model="form.message" type="text" class="input" />
          <InputFieldError :message="editForm.errors.message" />
        </div>

        <div class="mb-2" v-for="(segment, index) in form.tones" :key="index">
          <label class="label mb-1">{{ trans(`Tone ${index + 1}`) }}</label>
          <input v-model="segment.text" type="text" class="input" />
          <InputFieldError :message="form.errors[`tones.${index}.text`]" />
        </div>
  ...
  <div id="editItemDrawer" class="drawer drawer-right">
    <form @submit.prevent="update">
      ...
      <div class="drawer-body">
        ...
        <div class="mb-2">
          <label class="label mb-1">{{ trans('Message') }}</label>
          <input v-model="editForm.message" type="text" class="input" />
          <InputFieldError :message="editForm.errors.message" />
        </div>

        <div class="mb-2" v-for="(segment, index) in editForm.tones" :key="index">
          <label class="label mb-1">{{ trans(`Tone ${index + 1}`) }}</label>
          <input v-model="segment.text" type="text" class="input" />
          <InputFieldError :message="editForm.errors[`tones.${index}.text`]" />
        </div>
