...
const props = defineProps(['seo', 'authPages', 'googleClient', 'facebookClient'])
    ...
    <div class="signin-banner-from d-flex justify-content-center align-items-center">
      <div class="signin-banner-from-wrap">
        ...
        <div class="d-flex flex-column flex-sm-row signin-banner-login-browser gap-3">
          <a class="d-flex align-items-center justify-content-center mb-md-0 flex-fill mb-2 gap-1 text-nowrap"
            href="/auth/google" v-if="googleClient">
          ...
          <a class="d-flex align-items-center justify-content-center mb-md-0 flex-fill mb-2 gap-1" href="/auth/facebook"
            v-if="facebookClient">
        ...
        <div class="signin-banner-from-box">
          ...
          <form @submit.prevent="submit">
            <div class="row">
              ...
              <div class="col-12">
                <div class="postbox__comment-input mb-3">
                  ...
                  <input id="myInput" class="inputText password" v-model="form.password"
                    :type="showPassword ? 'text' : 'password'" required />

                  <span class="floating-label">{{ trans('Password') }}</span>
                  <button type="button" @click="showPassword = !showPassword" class="eye-btn">
                    <Icon icon="material-symbols:visibility-rounded" class="fs-5" v-if="showPassword" />
                    <Icon icon="material-symbols:visibility-off-rounded" class="fs-5" v-if="!showPassword" />
                  </button>
                ...
                <div class="col-6">
                  <div class="postbox__comment-agree">
                    <div class="form-check">
                      <input class="form-check-input" v-model="form.remember" type="checkbox" value=""
                        id="flexCheckDefault" />
            ...
            <div class="signin-banner-from-btn mb-20">
              <SpinnerBtn :processing="form.processing" class="signin-btn w-100" :btn-text="trans('Sign In')" />
            </div>
          </form>

          <div class="signin-banner-from-register mt-5">
            <Link href="/register">{{ trans("Don't have account ?") }} <span>{{ trans('Register') }}</span></Link>
          </div>
        </div>
      </div>
    </div>
